...
int main()
{
	...
	string numSongsPurchased; // Printout for song/s 
	...
	while (true)
	{
		...
		cin >> packageChoice; // User selection of listed packages
		...
		case 'c':
			...
			break;
		default: cout << "\n\t*** Sorry we do not offer that packet!"; // Message shown for any choice that is not a/A, b/B, c/C.
		...
		else // If Number of songs is neither a negative number or a non-integer
		{
			...
			cout // Message displaying the listed amount of the bill first separate (pre-tax amount and tax amount) and then total
				<< "\nYour total bill for this month is $" << setw(5) << setprecision(2) << fixed << totalPreTax  // Formatting value to occupy 5 spaces and having 2 decimal points after the dot.
				<< " + $" << setw(5) << setprecision(2) << fixed << taxAmount
				<< " (for tax) = " << setprecision(2) << fixed << totalBill << ".";
			...
			if (user_extra_songs > 0) // If the number of extra songs is more than 0
			{
				...
				cout // If there are any extra songs downloaded this message will print
					<< "\n\tYou have $" << setw(5) << setprecision(2) << fixed << user_extra_songs*user_additional_rate << " extra charges for having "
					<< user_extra_songs << " more " << numSongsPurchased
